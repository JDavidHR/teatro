...
if(isset($_GET['id']) && !empty($_GET['id'])){
    ...
    $ActualizarEstado = $mysql->efectuarConsulta("update actores set estado = 1 where id =".$idActor.""); 
    //Desconecto la conexion de la bD
    $mysql->desconectar(); 
    //decision para comprobar si se ejecuto, se redirige al index principal
    if($ActualizarEstado){
        //redireccion
       header("Location: ../ver_actor.php");
    } else {
        //mensaje de error

        echo "<div class=\"alert alert-warning alert-dismissible\"><a href=\"../ver_actor_inactivo.php\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a><strong>Alerta!</strong> No se ha podido habilitar el actor.</div>";
        //redireccion
        header( "refresh:3;url=../ver_actor_inactivo.php" );
    }
}else{
